...
{ /* -- Console ------------------------------------------------------------ */
  ...
  { // if window not available? This should never happen but we will put
    ...
    // Init input settings
    InputSetLockKeyModEnabled(cCVars->GetInternal<bool>(INP_LOCKKEYMODS));
    InputSetRawMouseEnabled(cCVars->GetInternal<bool>(INP_RAWMOUSE));
    InputSetStickyKeyEnabled(cCVars->GetInternal<bool>(INP_STICKYKEY));
    InputSetStickyMouseEnabled(cCVars->GetInternal<bool>(INP_STICKYMOUSE));
  ...
  Input() :
    ...
    EvtMainRegAuto{ cEvtMain, {        // Events list to register
      ...
      { EMC_INP_MOUSE_MOVE,   bind(&Input::InputOnMouseMove,   this, _1) },
      { EMC_INP_MOUSE_CLICK,  bind(&Input::InputOnMouseClick,  this, _1) },
      { EMC_INP_MOUSE_FOCUS,  bind(&Input::InputOnMouseFocus,  this, _1) },
      { EMC_INP_MOUSE_SCROLL, bind(&Input::InputOnMouseWheel,  this, _1) },
      { EMC_INP_KEYPRESS,     bind(&Input::InputOnKeyPress,    this, _1) },
      { EMC_INP_DRAG_DROP,    bind(&Input::InputOnDragDrop,    this, _1) },
      { EMC_INP_JOY_STATE,    bind(&Joystick::JoyOnState,      this, _1) },
    } },
    /* -- More initialisers ------------------------------------------------ */
