...
// ? default is '0' which causes a thread yield every frame but no delay. A
// ? value of '-1' completely disables any suspend or yield which may cause
// ? or alleviate problems with other engine threads. The maximum value is
// ? '1000' which means delay the engine thread every one second.
/* ------------------------------------------------------------------------- */
{ CFL_BASIC, "app_delay", cCommon->CommonZero(),
  CB(cTimer->TimerSetDelay, int), TINTEGERSAVE|PANY },
/* ------------------------------------------------------------------------- */
// ! APP_TITLE
