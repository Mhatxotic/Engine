...
{ /* -- Values storage ------------------------------------------ */ protected:
  ...
  template<typename AnyType> requires is_integral_v<IntType>
    constexpr void FlagSetInt(const AnyType atValue) noexcept
  { itV = static_cast<IntType>(atValue); }
  /* -- Swap values -------------------------------------------------------- */
  constexpr void FlagSwapStorage(FlagsStorageUnsafe &fcValue) noexcept
    { swap(itV, fcValue.itV); }
  /* -- Implicit init constructor (todo, make explicit but many errors!) --- */
  explicit constexpr FlagsStorageUnsafe(const IntType itValue) noexcept :
    /* -- Initialisers ----------------------------------------------------- */
    itV{ itValue }
    /* -- No code ---------------------------------------------------------- */
    {}
  ...
  template<typename AnyType=IntType>requires is_integral_v<IntType>
    constexpr AnyType FlagGet() const noexcept
...
requires is_integral_v<IntType> && is_std_atomic_v<SafeType>
class FlagsStorageSafe :
  /* -- Base classes ------------------------------------------------------- */
  private SafeType
{ /* -- Set values ---------------------------------------------- */ protected:
  template<typename AnyType>void FlagSetInt(const AnyType atValue) noexcept
    { this->store(static_cast<IntType>(atValue)); }
  /* -- Swap values -------------------------------------------------------- */
  void FlagSwapStorage(FlagsStorageSafe &fcValue) noexcept { swap(fcValue); }
  /* -- Implicit init constructor (todo, make explicit but many errors!) --- */
  explicit FlagsStorageSafe(const IntType itValue) noexcept :
    SafeType{ itValue } {}
  /* -- Get values ------------------------------------------------- */ public:
  template<typename AnyType=IntType>AnyType FlagGet() const noexcept
...
{ /* -- Is bits lesser than specified value? ----------------------- */ public:
  constexpr bool FlagIsLesser(const FlagsConst &fcValue) const noexcept
    { return this->FlagGet() < fcValue.FlagGet(); }
  /* -- Is bits lesser or equal to specified value? ------------------------ */
  constexpr bool FlagIsLesserEqual(const FlagsConst &fcValue) const noexcept
    { return this->FlagGet() <= fcValue.FlagGet(); }
  /* -- Is bits greater to specified value? -------------------------------- */
  constexpr bool FlagIsGreater(const FlagsConst &fcValue) const noexcept
    { return this->FlagGet() > fcValue.FlagGet(); }
  /* -- Is bits greater or equal to specified value? ----------------------- */
  constexpr bool FlagIsGreaterEqual(const FlagsConst &fcValue) const noexcept
    { return this->FlagGet() >= fcValue.FlagGet(); }
  /* -- Are there not any flags set? --------------------------------------- */
  constexpr bool FlagIsZero() const noexcept
    { return this->FlagGet() == static_cast<IntType>(0); }
  /* -- Are any flags actually set? ---------------------------------------- */
  constexpr bool FlagIsNonZero() const noexcept { return !FlagIsZero(); }
  /* -- Is flag set with specified value? ---------------------------------- */
  constexpr bool FlagIsSet(const FlagsConst &fcValue) const noexcept
    { return (this->FlagGet() & fcValue.FlagGet()) == fcValue.FlagGet(); }
  /* -- Is any flag set with specified value? ------------------------------ */
  constexpr bool FlagIsAnyOfSet(const FlagsConst &fcValue) const noexcept
    { return (this->FlagGet() & fcValue.FlagGet()) != 0; }
  /* -- Is bit clear of specified value? ----------------------------------- */
  constexpr bool FlagIsClear(const FlagsConst &fcValue) const noexcept
    { return (~this->FlagGet() & fcValue.FlagGet()) == fcValue.FlagGet(); }
  /* -- Is bit clear of specified value? ----------------------------------- */
  constexpr bool FlagIsAnyOfClear(const FlagsConst &fcValue) const noexcept
    { return (~this->FlagGet() & fcValue.FlagGet()) != 0; }
  /* -- Is flag set with specified value and clear with another? ----------- */
  constexpr bool FlagIsSetAndClear(const FlagsConst &fcSet,
    const FlagsConst &fcClear) const noexcept
      { return FlagIsSet(fcSet) && FlagIsClear(fcClear); }
  /* -- Flags are not masked in specified other flags? --------------------- */
  constexpr bool FlagIsNotInMask(const FlagsConst &fcValue) const noexcept
    { return this->FlagGet() & ~fcValue.FlagGet(); }
  /* -- Flags are masked in specified other flags? ------------------------- */
  constexpr bool FlagIsInMask(const FlagsConst &fcValue) const noexcept
    { return !FlagIsNotInMask(fcValue); }
  /* -- Is any of these flags set and cleared? ----------------------------- */
  constexpr bool FlagIsAnyOfSetAndClear() const noexcept { return false; }
  template<typename ...VarArgs>
    constexpr bool FlagIsAnyOfSetAndClear(const FlagsConst &fcSet,
      const FlagsConst &fcClear, const VarArgs ...vaArgs) const noexcept
  ...
  constexpr bool FlagIsEqualToBool(const FlagsConst &fcValue,
    const bool bState) const noexcept
  ...
  template<typename AnyType>
    constexpr const AnyType FlagIsSetTwo(const FlagsConst &fcValue,
      const AnyType atSet, const AnyType atClear) const noexcept
        { return FlagIsSet(fcValue) ? atSet : atClear; }
  /* -- Init constructors -------------------------------------------------- */
  template<typename AnyType>
    constexpr explicit FlagsConst(const AnyType atValue) noexcept :
      StorageType{ static_cast<IntType>(atValue) } {}
  /* -- Operators ---------------------------------------------------------- */
  constexpr const FlagsConst operator~() const noexcept
    { return FlagsConst{ ~this->template FlagGet<IntType>() }; }
  constexpr const FlagsConst operator|(const FlagsConst &fcRHS) const noexcept
    { return FlagsConst{ this->template FlagGet<IntType>() |
...
struct Flags :
  ...
  public ConstType
{ /* -- Swap function ------------------------------------------------------ */
  constexpr void FlagSwap(Flags &fValue) noexcept
    { this->FlagSwapStorage(fValue); }
  /* -- Set bits ----------------------------------------------------------- */
  constexpr void FlagSet(const IntType &itOther) noexcept
    { this->FlagSetInt(this->FlagGet() | itOther); }
  constexpr void FlagSet(const ConstType &ctValue) noexcept
    { this->FlagSetInt(this->FlagGet() | ctValue.FlagGet()); }
  /* -- Set all bits ------------------------------------------------------- */
  constexpr void FlagReset(const IntType itOther) noexcept
    { this->FlagSetInt(itOther); }
  constexpr void FlagReset(const ConstType &ctValue) noexcept
    { this->FlagSetInt(ctValue.FlagGet()); }
  constexpr void FlagReset() noexcept { this->FlagSetInt(0); }
  /* -- Not specified bits ------------------------------------------------- */
  constexpr void FlagNot(const ConstType &ctValue) noexcept
    { this->FlagSetInt(this->FlagGet() & ~ctValue.FlagGet()); }
  /* -- Mask bits ---------------------------------------------------------- */
  constexpr void FlagMask(const ConstType &ctValue) noexcept
    { this->FlagSetInt(this->FlagGet() & ctValue.FlagGet()); }
  /* -- Toggle specified bits ---------------------------------------------- */
  constexpr void FlagToggle(const ConstType &ctValue) noexcept
    { this->FlagSetInt(this->FlagGet() ^ ctValue.FlagGet()); }
  /* -- Clear specified bits ----------------------------------------------- */
  constexpr void FlagClear(const ConstType &ctValue) noexcept
    { FlagMask(ConstType{ ~ctValue.template FlagGet<IntType>() }); }
  /* -- Add or clear bits -------------------------------------------------- */
  constexpr void FlagSetOrClear(const ConstType &ctValue,
    const bool bCondition) noexcept
  ...
  constexpr void FlagSetAndClear(const ConstType &ctSet,
    const ConstType &ctClear) noexcept
  ...
  constexpr void FlagClearAndSet(const ConstType &ctClear,
    const ConstType &ctSet) noexcept
      { FlagClear(ctClear); FlagSet(ctSet); }
  /* -- Init constructors -------------------------------------------------- */
  constexpr explicit Flags(const IntType &itOther) noexcept :
    ConstType{ itOther } {}
  constexpr explicit Flags(const ConstType &ctOther) noexcept :
   ConstType{ ctOther } {}
  /* -- Default constructor ------------------------------------------------ */
...
{ /* -- Implicit init constructor (todo, make explicit!) ----------- */ public:
  explicit SafeFlags(const ConstType &fcValue) noexcept :
    FlagsType{ fcValue } {}
  explicit SafeFlags(const UConstType &fcValue) noexcept :
    FlagsType{ fcValue } {}
};/* ----------------------------------------------------------------------- */
/* == Flags helper macro =================================================== */
...
constexpr uint64_t Flag(const size_t stIndex) noexcept
